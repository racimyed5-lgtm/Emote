...
class CLIENT:
            ...
            try:
                if not command_queue.empty():
                    ...
                    if command_type == 'join_and_emote':
                        ...
                        # Perform emotes for all UIDs
                        for uid in uids:
                            if uid:
                                print(f"[API] Performing emote {emote_id} for UID: {uid}")
                                emote_packet = await Emote_k(uid, emote_id, self.key, self.iv)
                                await self.SEndPacKeT(self.whisper_writer, self.online_writer, 'OnLine', emote_packet)
                        ...
                        print(f"[API] Command completed successfully")
                    ...
                    if self.data2:
                        ...
                        if self.data2.hex().startswith("0500") and self.insquad is None and joining_team == False:
                            ...
                            try:
                                ...
                                RefUse = await masryRefuse(squad_owner, invite_uid, key, iv)
                                await self.SEndPacKeT(self.whisper_writer, self.online_writer, 'OnLine', RefUse)
                                SendInv = await masry_SendInv(invite_uid, key, iv)
                            ...
                            if self.insquad == False:
                                Join = await masryAccepted(squad_owner, squad_code, key, iv)
    ...
    async def MaiiiinE(self):
        ...
        print(render('MASRY API', colors=['white', 'red'], align='center'))
...
def run_flask():
    ...
    app.run(host='0.0.0.0', port=13699, debug=False, use_reloader=False)
...
async def StarTinG():
    ...
    flask_thread.start()
    print("[Flask] API server starting on http://0.0.0.0:5000")
