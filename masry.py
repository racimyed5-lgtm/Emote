...
def GeT_Name(uid , Token):
    ...
    if response.status_code == 200 or 201:
        ...
        masry_data = json.loads(DeCode_PackEt(packet))      
        try:
            a1 = masry_data["1"]["data"]["3"]["data"]
            return a1
        except: return ''  
    else: return ''
...
def GeT_PLayer_InFo(uid , Token):
    ...
    if response.status_code == 200 or 201:
        ...
        NoCLan = False   
        try:        
            a1 = str(masry_data["1"]["data"]["1"]["data"])
            a2 = masry_data["1"]["data"]["21"]["data"]
            a3 = masry_data["1"]["data"]["3"]["data"]
            player_server = masry_data["1"]["data"]["5"]["data"]
            player_bio = masry_data["9"]["data"]["9"]["data"]
            player_level = masry_data["1"]["data"]["6"]["data"]
            account_date = datetime.fromtimestamp(masry_data["1"]["data"]["44"]["data"]).strftime("%I:%M %p - %d/%m/%y")
            last_login = datetime.fromtimestamp(masry_data["1"]["data"]["24"]["data"]).strftime("%I:%M %p - %d/%m/%y")
            try:
                clan_id = masry_data["6"]["data"]["1"]["data"]
                clan_name = masry_data["6"]["data"]["2"]["data"]
                clan_leader = masry_data["6"]["data"]["3"]["data"]
                clan_level = masry_data["6"]["data"]["4"]["data"]
                clan_members_num = masry_data["6"]["data"]["6"]["data"]
                clan_leader_name = masry_data["7"]["data"]["3"]["data"]                       
            except:
                NoCLan = True
            if NoCLan:
                a = f'''
